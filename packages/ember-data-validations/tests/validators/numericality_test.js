...
test('when only allowing integers and value is not integer', function() {
  ...
  equal(DS.Validations.validators.local.numericality(model, 'attribute', options), 'failed integer validation');
});
...
test('when only allowing values equal to 10 and value is 11', function() {
  ...
  equal(DS.Validations.validators.local.numericality(model, 'attribute', options), 'failed equal validation');
});
...
test('when options is true', function() {
  options = true;
  equal(DS.Validations.validators.local.numericality(model, 'attribute', options), 'is not a number');
});

test('when only integer and no message is passed', function() {
  model.set('attribute', 1.1);
  options = { only_integer: true };
  equal(DS.Validations.validators.local.numericality(model, 'attribute', options), 'must be an integer');
});

test('when equal to  and no message is passed', function() {
  model.set('attribute', 10);
  options = { equal_to: 11 };
  equal(DS.Validations.validators.local.numericality(model, 'attribute', options), 'must be equal to 11');
});

test('when greater than and no message is passed', function() {
  model.set('attribute', 10);
  options = { greater_than: 11 };
  equal(DS.Validations.validators.local.numericality(model, 'attribute', options), 'must be greater than 11');
});

test('when greater than or equal to and no message is passed', function() {
  model.set('attribute', 10);
  options = { greater_than_or_equal_to: 11 };
  equal(DS.Validations.validators.local.numericality(model, 'attribute', options), 'must be greater than or equal to 11');
});

test('when less than and no message is passed', function() {
  model.set('attribute', 11);
  options = { less_than: 10 };
  equal(DS.Validations.validators.local.numericality(model, 'attribute', options), 'must be less than 10');
});

test('when less than or equal to and no message is passed', function() {
  model.set('attribute', 11);
  options = { less_than_or_equal_to: 10 };
  equal(DS.Validations.validators.local.numericality(model, 'attribute', options), 'must be less than or equal to 10');
});

test('when odd and no message is passed', function() {
  model.set('attribute', 10);
  options = { odd: true };
  equal(DS.Validations.validators.local.numericality(model, 'attribute', options), 'must be odd');
});

test('when even and no message is passed', function() {
  model.set('attribute', 11);
  options = { even: true };
  equal(DS.Validations.validators.local.numericality(model, 'attribute', options), 'must be even');
});
