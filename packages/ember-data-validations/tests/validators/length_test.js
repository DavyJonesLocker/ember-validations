...
test('when options is a number', function() {
  model.set('attribute', '1234');
  options = 3;
  equal(DS.Validations.validators.local.length(model, 'attribute', options), 'is the wrong length (should be 3 characters)');
});
...
test('when allowed length minimum is 3, value length is 2 and no message is set', function() {
  model.set('attribute', '12');
  options = { minimum: 3 };
  equal(DS.Validations.validators.local.length(model, 'attribute', options), 'is too short (minimum is 3 characters)');
});

test('when allowed length maximum is 3, value length is 4 and no message is set', function() {
  model.set('attribute', '1234');
  options = { maximum: 3 };
  equal(DS.Validations.validators.local.length(model, 'attribute', options), 'is too long (maximum is 3 characters)');
});
